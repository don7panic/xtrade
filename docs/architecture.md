...
- **高质量数据展示**：TUI 界面提供实时价格、OrderBook、24h 统计、价格趋势折线图（时间轴线图）与通知区域。
...
- **Price Trend Panel**：使用折线图呈现价格趋势，横轴按照采样时间顺序排列，纵轴基于最近窗口内的最小/最大价格做动态归一化，保证微小波动也可在终端像素内清晰辨识；配置项 `enable_sparkline` 继续作为该折线图的启用开关以保持兼容。
